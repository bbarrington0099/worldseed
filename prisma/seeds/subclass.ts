...
export async function seedSubclasses(
	...
	return {
		...
		cleric_arcana_domain: await db.createSubclass({
			...
			name: 'Arcana Domain',
			description: 'Scholar-clerics who bridge divine power with arcane magic, guarding ancient libraries and protecting the secrets of the world.',
			playstyle:
		...
		cleric_death_domain: await db.createSubclass({
			...
			name: 'Death Domain',
			description: 'Clerics who serve death gods and command the forces of undeath, guiding souls to their proper rest while wielding necrotic power.',
			playstyle:
		...
		cleric_grave_domain: await db.createSubclass({
			...
			name: 'Grave Domain',
			description: 'Clerics who protect the balance between life and death, tending tombs and aiding those on the brink while preventing unnatural resurrection.',
			playstyle:
		...
		cleric_knowledge_domain: await db.createSubclass({
			...
			description: 'Sages and librarians who preserve lore and serve as oracles, gathering information and using divination to guide their allies.',
			playstyle:
		...
		cleric_order_domain: await db.createSubclass({
			...
			name: 'Order Domain',
			description: 'Clerics who serve gods of law and enforce decrees, embodying discipline while fighting for justice and bolstering allies with commanding spells.',
			playstyle:
		...
		cleric_peace_domain: await db.createSubclass({
			...
			name: 'Peace Domain',
			description: 'Mediators who prevent violence and unite divided communities, invoking holy bonds that unite allies and dampen strife.',
			playstyle:
		...
		cleric_twilight_domain: await db.createSubclass({
			...
			description: 'Guardians who watch over the border between light and darkness, protecting travelers and providing shelter in the dangerous hours of twilight.',
			playstyle:
		...
		druid_circle_of_dreams: await db.createSubclass({
			...
			description: 'Druids who commune with fey realms through magical anomalies, using gentle restoration magic and fey vitality to heal and protect allies.',
			playstyle:
		...
		druid_circle_of_the_shepherd: await db.createSubclass({
			...
			description: 'Guardians of the wild who care for nature\'s herds and command spiritual beasts, calling forth powerful allies to fight alongside them.',
			playstyle:
		...
		druid_circle_of_spores: await db.createSubclass({
			...
			description: 'Druids who study fungal magic and understand that decay is part of the life cycle, using spores to both heal and harm while finding beauty in rot and renewal.',
			playstyle:
		...
		druid_circle_of_stars: await db.createSubclass({
			...
			description: 'Druids who chart the cosmos and navigate by the stars, using starlight to guide and bless their allies with celestial power.',
			playstyle:
		...
		fighter_arcane_archer: await db.createSubclass({
			...
			name: 'Arcane Archer',
			description: 'Fighters who enchant their arrows with arcane power, creating magical shots that can strike from distance with devastating effects.',
			playstyle:
		...
		fighter_cavalier: await db.createSubclass({
			...
			name: 'Cavalier',
			description: 'Stalwart defenders who excel at mounted combat and protecting allies, serving lords with unwavering loyalty and leading cavalry charges.',
			playstyle:
		...
		fighter_echo_knight: await db.createSubclass({
			...
			name: 'Echo Knight',
			description: 'Warriors who harness echoing strands of time to create spectral duplicates, striking from multiple positions with unpredictable precision.',
			playstyle:
		...
		fighter_psi_warrior: await db.createSubclass({
			...
			name: 'Psi Warrior',
			description: 'Disciplined soldiers who use mind over matter powers to enhance their combat effectiveness, empowering strikes with psychic energy.',
			playstyle:
		...
		fighter_rune_knight: await db.createSubclass({
			...
			name: 'Rune Knight',
			description: 'Warriors who etch magical runes into their armor and weapons, tapping into giant power and earth magic to enhance their battlefield strength.',
			playstyle:
		...
		fighter_samurai: await db.createSubclass({
			...
			name: 'Samurai',
			description: 'Warriors of honor who serve lords with unyielding spirit, focusing their will to endure and attack fiercely with disciplined precision.',
			playstyle:
		...
		monk_way_of_the_astral_self: await db.createSubclass({
			...
			description: 'Monks who commune with their inner spirits and the astral plane, manifesting astral arms for extra reach and control in combat.',
			playstyle:
		...
		monk_way_of_the_ascendant_dragon: await db.createSubclass({
			...
			description: 'Monks who channel draconic might into their strikes, studying legends of draconic power and augmenting attacks with dragon breaths and scales.',
			playstyle:
		...
		monk_way_of_the_drunken_master: await db.createSubclass({
			...
			description: 'Light-footed fighters who use unpredictable movements to confound foes, dodging and striking with chaotic agility that bewilders enemies.',
			playstyle:
		...
		monk_way_of_the_kensei: await db.createSubclass({
			...
			description: 'Disciplined fighters who perfect weapon arts alongside unarmed combat, combining monk martial arts with weapon mastery seamlessly.',
			playstyle:
		...
		monk_way_of_the_long_death: await db.createSubclass({
			...
			description: 'Monks who meditate on mortality and channel death energy, learning to frighten foes with terror while understanding the boundaries of life and death.',
			playstyle:
		...
		monk_way_of_mercy: await db.createSubclass({
			...
			name: 'Way of Mercy',
			description: 'Traveling healers who mix healing touch with lethal strikes, dealing pain or mending wounds as needed while defending themselves and others.',
			playstyle:
		...
		monk_way_of_the_sun_soul: await db.createSubclass({
			...
			description: 'Illuminated fighters who channel radiance into energy, striking with beams of light in addition to melee attacks with celestial power.',
			playstyle:
		...
		paladin_oath_of_conquest: await db.createSubclass({
			...
			description: 'Paladins who serve conquerors and tyrants, imposing order through strength and fear while controlling the battlefield with punishing blows.',
			playstyle:
		...
		paladin_oath_of_glory: await db.createSubclass({
			...
			name: 'Oath of Glory',
			description: 'Champions inspired by heroic tales who strive for legendary feats, inspiring allies with courage while fighting on the front lines.',
			playstyle:
		...
		paladin_oath_of_redemption: await db.createSubclass({
			...
			description: 'Pacifists who seek to prevent violence through redemption rather than destruction, converting enemies away from violence and preventing damage through protective means.',
			playstyle:
		...
		paladin_oath_of_the_watchers: await db.createSubclass({
			...
			description: 'Guardians who protect borders from otherworldly invasion, becoming vigilant defenders against supernatural and extraplanar enemies.',
			playstyle:
		...
		ranger_drakewarden: await db.createSubclass({
			...
			name: 'Drakewarden',
			description: 'Rangers who form bonds with draconic companions and channel draconic energy to empower their arrows with fiery attacks.',
			playstyle:
		...
		ranger_fey_wanderer: await db.createSubclass({
			...
			name: 'Fey Wanderer',
			description: 'Enchanted rangers who gain fey abilities through magical anomalies, using illusions and charm to confuse enemies while blending wilderness survival.',
			playstyle:
		...
		ranger_horizon_walker: await db.createSubclass({
			...
			name: 'Horizon Walker',
			description: 'Masters of the Astral Plane who patrol world borders and step between planes, hunting extraplanar threats with teleporting strikes.',
			playstyle:
		...
		ranger_monster_slayer: await db.createSubclass({
			...
			name: 'Monster Slayer',
			description: 'Hunters of creatures who specialize in single-target damage against supernatural enemies, using supernatural senses to track and overcome monstrous foes.',
			playstyle:
		...
		ranger_swarmkeeper: await db.createSubclass({
			...
			name: 'Swarmkeeper',
			description: 'Rangers who form connections with swarms of nature, commanding insects or vermin to harass enemies while moving through the battlefield.',
			playstyle:
		...
		rogue_inquisitive: await db.createSubclass({
			...
			name: 'Inquisitive',
			description: 'Detectives and spies who seek out secrets and track lies with acute perception, excelling at noticing hidden things and countering deception.',
			playstyle:
		...
		rogue_mastermind: await db.createSubclass({
			...
			name: 'Mastermind',
			description: 'Criminal strategists who manipulate others and understand tactics, focusing on allies with cunning action, support, and manipulation.',
			playstyle:
		...
		rogue_phantom: await db.createSubclass({
			...
			name: 'Phantom',
			description: 'Rogues who blur the lines between life and death, channeling spirits to deceive and terrify while siphoning damage with ghostly powers.',
			playstyle:
		...
		rogue_scout: await db.createSubclass({
			...
			name: 'Scout',
			description: 'Patrollers with wilderness expertise who dodge effortlessly and strike quickly from cover, using hit-and-run tactics to outmaneuver enemies.',
			playstyle:
		...
		rogue_soulknife: await db.createSubclass({
			...
			name: 'Soulknife',
			description: 'Mystic assassins who create blades of pure mind through psychic energy, using telepathic blades and psychic skills for silent kills.',
			playstyle:
		...
		rogue_swashbuckler: await db.createSubclass({
			...
			name: 'Swashbuckler',
			description: 'Charismatic duelists who blend panache with swordplay, charming allies and foes alike while focusing on single combat with flair and mobility.',
			playstyle:
		...
		sorcerer_aberrant_mind: await db.createSubclass({
			...
			name: 'Aberrant Mind',
			description: 'Sorcerers haunted by psionic powers from strange cosmic energies, blending sorcery with mind-bending powers to confuse and control foes.',
			playstyle:
		...
		sorcerer_clockwork_soul: await db.createSubclass({
			...
			name: 'Clockwork Soul',
			description: 'Sorcerers who embody cosmic order, drawing upon mechanical precision and cosmic balance to grant stability to allies through order-infused spells.',
			playstyle:
		...
		sorcerer_lunar_sorcery: await db.createSubclass({
			...
			name: 'Lunar Sorcery',
			description: 'Sorcerers attuned to the moon who call forth moonlight to empower their magic, boosting spells by harnessing lunar phases.',
			playstyle:
		...
		sorcerer_shadow_magic: await db.createSubclass({
			...
			name: 'Shadow Magic',
			description: 'Sorcerers born under darkness who wield shadows as their power source, using darkness and necrotic spells for control and stealth.',
			playstyle:
		...
		sorcerer_storm_sorcery: await db.createSubclass({
			...
			name: 'Storm Sorcery',
			description: 'Sorcerers who draw on storm and sky to cast thunderous spells, specializing in lightning and thunder magic with elemental fury.',
			playstyle:
		...
		warlock_the_celestial: await db.createSubclass({
			...
			name: 'The Celestial',
			description: 'Warlocks who serve patrons from the Upper Planes, gaining healing and light magic to aid allies while dealing radiant damage to foes.',
			playstyle:
		...
		warlock_the_fathomless: await db.createSubclass({
			...
			name: 'The Fathomless',
			description: 'Warlocks bound to entities of the deep seas or underground oceans, gaining mysterious sea powers to manipulate water and tentacles to bind enemies.',
			playstyle:
		...
		warlock_the_genie: await db.createSubclass({
			...
			name: 'The Genie',
			description: 'Warlocks bound to genie patrons from the Elemental Planes, gaining elemental might and plane-travel abilities for versatile casting.',
			playstyle:
		...
		warlock_the_hexblade: await db.createSubclass({
			...
			name: 'The Hexblade',
			description: 'Warlocks bound to sentient weapons or curses, blending martial and magic by cursing their foes and striking with cursed weapons.',
			playstyle:
		...
		warlock_the_undead: await db.createSubclass({
			...
			name: 'The Undead',
			description: 'Warlocks who make dark pacts with necromantic powers, gaining control over deathly energies to raise undead and drain life with spells.',
			playstyle:
		...
		warlock_the_undying: await db.createSubclass({
			...
			name: 'The Undying',
			description: 'Warlocks who serve ageless beings seeking the secrets of immortality, resisting death effects and sustaining life through arcane means.',
			playstyle:
		...
		wizard_school_of_abjuration: await db.createSubclass({
			...
			description: 'Wizards who specialize in protective magic, excelling at shields, protective wards, and dispelling enemy magic as magical guardians.',
			playstyle:
		...
		wizard_school_of_bladesinging: await db.createSubclass({
			...
			description: 'Elven wizards who perform martial dance with arcane precision, blending swordplay with spells to combine agility and magic seamlessly.',
			playstyle:
		...
		wizard_school_of_chronurgy: await db.createSubclass({
			...
			description: 'Wizards who master time itself, studying the flow of time and bending it to their will to alter fate and manipulate outcomes in battle.',
			playstyle:
		...
		wizard_school_of_conjuration: await db.createSubclass({
			...
			description: 'Wizards who act as portals through the multiverse, summoning creatures and objects to adapt to situations with versatile casting.',
			playstyle:
		...
		wizard_school_of_enchantment: await db.createSubclass({
			...
			description: 'Wizards who weave charm and illusion into their spells, swaying minds and hearts to excel at charming enemies and controlling crowds.',
			playstyle:
		...
		wizard_school_of_graviturgy: await db.createSubclass({
			...
			description: 'Wizards who manipulate gravity itself, making objects heavy or weightless to control the battlefield by warping the forces of nature.',
			playstyle:
		...
		wizard_school_of_necromancy: await db.createSubclass({
			...
			description: 'Wizards who commune with death and command undead or sap life, understanding mortality to raise undead, drain life, and endure undeath.',
			playstyle:
		...
		wizard_order_of_scribes: await db.createSubclass({
			...
			description: 'Scholars who inscribe spells into sentient spellbooks that guide them through magic, adapting easily with any prepared spell as their focus.',
			playstyle:
		...
		wizard_school_of_transmutation: await db.createSubclass({
			...
			description: 'Wizards who alter matter itself, turning stone to mud or man to bird to change the properties of matter and improve allies.',
			playstyle:
		...
		wizard_war_magic: await db.createSubclass({
			...
			description: 'Soldiers who blend spellcasting with martial prowess, guarding arcane battlefields with lethal spells while defending against enemy attacks.',
			playstyle:
